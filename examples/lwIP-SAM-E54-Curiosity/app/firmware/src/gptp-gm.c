...
static void OnSyncSent(TC6_t *pInst, const uint8_t *pTx, uint16_t len, void *pTag, void *pGlobalTag);
static void OnFollowUpSent(TC6_t *pInst, const uint8_t *pTx, uint16_t len, void *pTag, void *pGlobalTag);
...
static void SendSync(GptpGm_t *gm)
{
    ...
    if (TC6LwIP_SendRawEthernetPacket(gm->idx, gm->syncBuf, SYNC_FRAME_LEN, slot, OnSyncSent, gm)) {
        gm->syncPending = true;
...
static void OnSyncSent(TC6_t *pInst, const uint8_t *pTx, uint16_t len, void *pTag, void *pGlobalTag)
{
    /* Sync is on the wire; the HW TX timestamp arrives later via gPTP_GM_OnTxTimestamp. */
...
bool gPTP_GM_Init(int8_t idx)
{
    ...
    if ((idx >= 0) && (idx < TC6_MAX_INSTANCES)) {
        ...
        if (TC6LwIP_GetTimestampSupported(idx)) {
            TC6LwIP_GetMac(idx, &macPtr); /* macPtr -> internal 6-byte MAC */
            if (NULL != macPtr) {
